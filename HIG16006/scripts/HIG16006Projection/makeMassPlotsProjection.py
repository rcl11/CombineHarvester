    ...
    for CHN in ['et', 'mt', 'em', 'tt']:
        for CAT in ['8', '9']:
            ...
            os.system(('python scripts/postFitPlot.py' \
                  ' --file=ECFA_noshapes_normalrebin_shiftup_shapes_lumi100.root --ratio --extra_pad="%(PAD)s" --mPhi=700 --r_ggH=0.1 --r_bbH=0.1' \
                  ...
                  ' --channel_label "%(LABEL)s" --manual_blind --x_blind_min=0' % vars()))
...
for MODE in ['prefit']:
    for CHN in ['et', 'mt', 'em', 'tt']:
        for CAT in ['8', '9']:
            LABEL = "%s %s" % (CHN_DICT[CHN], CAT_DICT[CAT])
            os.system(('python scripts/postFitPlot.py' \
                  ' --file=ECFA_noshapes_normalrebin_shiftup_shapes_lumi100.root --ratio --extra_pad=0.6 --mPhi=700 --r_ggH=0.1 --r_bbH=0.1' \
                  ' --file_dir="htt_%(CHN)s_%(CAT)s" --custom_x_range --x_axis_min=0 --x_axis_max 200' \
                  ' --ratio_range 0.4,1.6 ' \
                  ' --outname htt_%(CHN)s_%(CAT)s --mode %(MODE)s' \
                  ' --channel_label "%(LABEL)s" --manual_blind --x_blind_min=0.0' % vars()))

...
#include "CombineHarvester/HIG16006/interface/HttSystematics_MSSMRun2Projection.h"
...
int main(int argc, char** argv) {
  ...
  config.add_options()
    ...
    ("hist_err_shift_up", po::value<bool>(&hist_err_shift_up)->default_value(false))
    ("hist_err_shift_down", po::value<bool>(&hist_err_shift_down)->default_value(false))
 ...
 //     {"mt"};
  ...
  ch::AddMSSMRun2ProjectionSystematics(cb,control_region);
  //! [part7]
  for (string chn:chns){
...
/*     cb.cp().process({"TTH_SM125"}).ForEachProc([&](ch::Process *proc){
  ...
  // This would be the time to alter the histograms
  if(hist_err_shift_up || hist_err_shift_down){
    if(hist_err_shift_up) {
      cb.ForEachProc([](ch::Process *p) {
        auto newhist = p->ClonedScaledShape();
        int idx = newhist->GetXaxis()->FindBin(200);
        for(int i=0; i<newhist->GetNbinsX(); i++) {
            if(i>=idx && (newhist->GetBinError(i)/newhist->GetBinContent(i))>0.5) newhist->SetBinContent(i,newhist->GetBinContent(i)+newhist->GetBinError(i));
        }
        p->set_shape(std::move(newhist), true);
      });
    }
    else if(hist_err_shift_down) {
      cb.ForEachProc([](ch::Process *p) {
        auto newhist = p->ClonedScaledShape();
        int idx = newhist->GetXaxis()->FindBin(200);
        for(int i=0; i<newhist->GetNbinsX(); i++) {
